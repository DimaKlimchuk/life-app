...
function startLiveUpdate() {
  ...
  function updateLifePercentage() {
    const birthdate = new Date("2004-03-16");
    const now = new Date();
    ...
    document.getElementById("daysLived").innerText = Math.floor(
      livedSeconds / 86400
    );
    document.getElementById("lifePercent").innerText = lifePercent.toFixed(8);
    document.getElementById("progress").style.width =
      lifePercent.toFixed(2) + "%";
  }
...
function renderTimers() {
  ...
  timers.forEach((timer) => {
    ...
    li.innerHTML = `
            ...
            <span class="timer-time" onclick="editTimer(${
              timer.id
            })" title="Click to edit">${formatTime(timer.seconds)}</span>
